...
is_applesilicon=$([[ "$(uname -m)" == "arm64" || "$(sysctl -n sysctl.proc_translated 2>/dev/null)" == "1" ]] && echo true || echo false)
...
langGenerateConfig="(%d) Generate config file only (manual install)"
...
langChooseResOp7="(7) 3440x1440 Display"
langChooseResOpCustom="(8) Manual input resolution"

langNoMonitFound="No monitors were found. Exiting..."
langMonitVIDPID="Your monitor VID:PID:"
langConfigGenerated="Config file generated successfully!"
langConfigLocation="File location:"
langManualInstallHint="Please copy the folder to:"
langManualInstallHint2="(requires sudo)"
...
langGenerateConfig="(%d) 仅生成配置文件（手动安装）"
  ...
  langChooseResOp7="(7) 3440x1440 显示屏"
  langChooseResOpCustom="(8) 手动输入分辨率"

langNoMonitFound="没有找到监视器。 退出..."
langMonitVIDPID="您的显示器 供应商ID:产品ID:"
langConfigGenerated="配置文件生成成功！"
langConfigLocation="文件位置："
langManualInstallHint="请将文件夹复制到："
langManualInstallHint2="（需要 sudo 权限）"
fi
...
function get_edid() {
  ...
  if [[ "${#gDisplayInf[@]}" -eq 0 ]]; then
      echo "尝试其他方法检测显示器..."
      # 尝试从不同的路径获取
      gDisplayInf=($(ioreg -c IODisplayConnect -r -l | grep -i "IODisplayEDID" | sed -e "/[^<]*</s///" -e "s/\>//"))
  fi
  
  # 如果还是没有，尝试第三种方法
  if [[ "${#gDisplayInf[@]}" -eq 0 ]]; then
      echo "尝试第三种方法..."
      gDisplayInf=($(system_profiler SPDisplaysDataType | grep -A 20 "Displays:" | grep -i "EDID" | sed -e "/[^<]*</s///" -e "s/\>//"))
  fi

  echo "检测到 ${#gDisplayInf[@]} 个显示器的 EDID 信息"
  ...
  if [[ "${#gDisplayInf[@]}" -ge 2 ]]; then
      ...
      for display in "${gDisplayInf[@]}"; do
          let index++
          # 安全检查 EDID 长度
          if [[ ${#display} -ge 256 ]]; then
              MonitorName=("$(echo ${display:190:24} | xxd -p -r 2>/dev/null || echo "Unknown Monitor")")
...
function get_vidpid_applesilicon() {
  ...
  if [[ "${#vends[@]}" -eq 0 ]]; then
      echo "尝试其他方法获取显示器信息..."
      vends=($(ioreg -c AppleBacklightDisplay -r -l | grep -o '"LegacyManufacturerID"=[0-9]*' | cut -d= -f2))
      prods=($(ioreg -c AppleBacklightDisplay -r -l | grep -o '"ProductID"=[0-9]*' | cut -d= -f2))
  fi
  
  # 如果还是失败，尝试方法3
  if [[ "${#vends[@]}" -eq 0 ]]; then
      echo "尝试第三种方法..."
      vends=($(ioreg -c IODisplayConnect -r -l | grep -o '"vendor-id"=<[^>]*>' | sed 's/.*<\([0-9a-f]*\)>.*/\1/' | while read hex; do printf "%d\n" 0x$hex; done))
      prods=($(ioreg -c IODisplayConnect -r -l | grep -o '"product-id"=<[^>]*>' | sed 's/.*<\([0-9a-f]*\)>.*/\1/' | while read hex; do printf "%d\n" 0x$hex; done))
  fi
  ...
  # 如果产品名称获取失败，尝试其他方法
  if [[ "${#prodnames[@]}" -eq 0 ]]; then
      IFS=$'\n' prodnames=($(system_profiler SPDisplaysDataType | grep -A1 "Display Type" | grep -v "Display Type" | sed 's/^[[:space:]]*//' | head -${#prods[@]}))
  fi
  set +o noglob
  ...
  if [[ "${#prods[@]}" -eq 0 ]]; then
      echo "未检测到任何显示器，尝试强制检测内置显示器..."
      # 为内置显示器设置默认值
      vends=(1552)  # Apple 的默认 VendorID
      prods=(41000) # 一个通用的 ProductID
      prodnames=("Built-in Display")
  fi
  ...
  if [[ "${#prods[@]}" -ge 2 ]]; then
      ...
      for prod in "${prods[@]}"; do
          if [[ $prodnamesindex -lt ${#prodnames[@]} ]]; then
              MonitorName=${prodnames[$prodnamesindex]}
          else
              MonitorName="Unknown Display"
          fi
...
function init() {
  ...
  if [[ $is_applesilicon == true ]]; then
      ...
      get_edid
      if [[ $? -ne 0 ]]; then
          echo "EDID 检测失败，尝试 Apple Silicon 方法..."
          get_vidpid_applesilicon
      fi
  fi
  ...
  if [[ -z $VendorID || -z $ProductID || $VendorID == 0 || $ProductID == 0 ]]; then
      ...
      echo "Pid: $Pid"
      echo ""
      echo "请尝试以下命令手动检查显示器信息："
      echo "ioreg -l | grep DisplayAttributes"
...
CCC
  ...
  4)
      ...
      ;;
  5)
      # 2560x1600 Display (16:10 aspect ratio)
      create_res_1 2560x1600 2304x1440 2048x1280 1920x1200 1792x1120 1680x1050 1440x900 1280x800
      create_res_2 1280x800 1280x720 960x600 960x540 640x360
      create_res_3 840x472 800x450 720x405 640x360 576x324 512x288 420x234 400x225 320x180
      create_res_4 2048x1280 1920x1200 1680x1050 1440x900 1280x800 1024x640 960x540 840x472 800x450 640x360
      ;;
  6)
...
# create resolution - 生成两种格式的 HiDPI 条目
# 数据结构: | 宽度 (32bit) | 高度 (32bit) | Scale Flags | Display Mode Flags |
# AAAAB = 简单 HiDPI 标志
# AAAABACAAAA== = HiDPI + 镜像有效 (kDisplayModeValidForMirroringFlag=0x00200000)
function create_res() {
  for res in $@; do
...
# create_res_1 - 基础分辨率列表，后缀 A（最简格式）
...
# create_res_2 - HiDPI + 镜像支持
# 后缀 AAAABACAAAA== 对应 flags: 00000001 00200000 (kDisplayModeValidForMirroringFlag)
...
# create_res_4 - 完整 HiDPI 标志（推荐使用）
# 后缀 AAAAJAKAAAA== 对应 flags: 00000001 00a00000
# 包含 kDisplayModeValidForMirroringFlag (0x00200000) + kDisplayModeValidForHiResFlag (0x00800000)
...
function generate_config_only() {
  ...
  local outputDir="${currentDir}/hidpi-config"
  mkdir -p "${outputDir}"

  cp -r ${currentDir}/tmp/DisplayVendorID-${Vid} "${outputDir}/"

  if [[ -f ${currentDir}/tmp/Icons.plist ]]; then
      cp ${currentDir}/tmp/Icons.plist "${outputDir}/"
  fi
  ...
  echo ""
  echo "========================================"
  echo "${langConfigGenerated}"
  echo "========================================"
  echo ""
  echo "${langConfigLocation}"
  echo "  ${outputDir}/DisplayVendorID-${Vid}/"
  echo ""
  echo "${langManualInstallHint}"
  echo "  sudo cp -r \"${outputDir}/DisplayVendorID-${Vid}\" \"/Library/Displays/Contents/Resources/Overrides/\""
  echo ""
  echo "${langManualInstallHint2}"
  echo ""
  echo "重启后生效。"
  echo ""
}
...
function start() {
  ...
  let opt++; printf "${langGenerateConfig}\n" $opt
  echo ""
